#!/usr/bin/env octave
%
% Hit the space bar to start, the escape key to end.
%
...
imageTypes = {'gabor'};
%imageTypes = {{'face','vehicle','animal'}};
imageBlockSecs = 12;
hemiBlockSecs = 12;
%imageTypes = {{'face','vehicle','animal'},{}};
%imageBlockSecs = 9;
%hemiBlockSecs = imageBlockSecs*2*3;
...
fprintf('Will draw %i image patches.\n', numImages);
fprintf('image block = %i seconds, hemifield block = %i seconds.\n\n', imageBlockSecs, hemiBlockSecs);
...
for(jj=1:numBlocks)
    if(isempty(imageTypes{jj}))
        % Special case for empty image- set up blank textures.
        imtex{jj}(1) = Screen('MakeTexture', win, zeros(sz), [], [], 2);
    elseif(ischar(imageTypes{jj}) && strcmpi(imageTypes{jj},'gabor'))
        % Define prototypical gabor patch size: s is half the desired size.
        phase = 0;
        sc = 5;
        freq = 0.05;
        tilt = 0;
        contrast = 5;
        sf = freq;
        a=cos(tilt*pi/180)*sf*360;
        b=sin(tilt*pi/180)*sf*360;
        multConst=1/(sqrt(2*pi)*sc);
        sinWave=sin((a*(gab_x - x) + b*(gab_y - y)+phase)*pi/180);
        varScale = varScale/2;
        ...
        imtex{jj}(1) = Screen('MakeTexture', win, m, [], [], 2);
    else
        im = loadCategoricalImages(numImages, imageTypes{jj});
        for(ii=1:numImages)
            if(~all(size(im{ii})==sz))
                ...
                curIm = imresize(double(im{ii}), sz, "linear");
            else
                curIm = double(im{ii});
            end
            m = (curIm./127.5 - 1.0) .* g;
...
inrect = repmat(texrect', 1, numImages);

dstRects = zeros(4, numImages);
for i=1:numImages
    scale(i) = (0.7 + 0.5 * randn);
    if(scale(i)<0.2) scale(i) = 0.2; end
...
rotAngles = rand(1, numImages) * 45;
driftAngles = rand(1, numImages) * 360;

count = 0;
curBlockNum = 1;
leftFlag = true;
leftRect = [0, 0, w/2, h];
rightRect = [0+w/2, 0, w, h];
...
Screen('FillOval', win, [0,255,0], [w/2-5,h/2-5,w/2+5,h/2+5]);
vbl = Screen('Flip', win);
sleep(0.5);
...
while(~keyCode(endKey))
    ...
    curTime = time();
    if((curTime-startImageSecs)>imageBlockSecs)
    ...
    if((curTime-startHemiSecs)>hemiBlockSecs)
        leftFlag = ~leftFlag;
        startHemiSecs = time();
    end
    ...
    if(leftFlag)
        Screen('FillRect', win, [128,128,128,255], leftRect);
        %Screen('DrawText', win, 'L', 30,30);
    else
        Screen('FillRect', win, [128,128,128,255], rightRect);
        %Screen('DrawText', win, 'R', 30,30);
    end
    Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE);
    ...
    rotAngles = rotAngles + randn(1, numImages);
    driftAngles = driftAngles + 10*randn(1, numImages);
