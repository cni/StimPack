...
imageTypes = {{'face','famous','vehicle','animal'},{}};
...
for(jj=1:numel(imageTypes))
    if(isempty(imageTypes{jj}))
        ...
        % Note: this will use a lot of memory uncessarily. Maybe rework code
        % to allow different numbers of imtex for the different categories?
        for(ii=1:numImages)
            imtex{jj}(ii) = Screen('MakeTexture', win, zeros(sz), [], [], 2);
        end
    else
